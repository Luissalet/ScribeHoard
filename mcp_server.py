"""Stdio MCP bridge for Scribe's Hoard.
...
app itself (`python -m scribe`, detached, on the port of SCRIBE_URL) and waits
for it, so a workspace that starts before the app still gets its tools;
SCRIBE_BRIDGE_AUTOSTART=0 turns that off.
"""
...
BASE_URL = os.environ.get("SCRIBE_URL", "http://127.0.0.1:5185").rstrip("/")
TOKEN_FILE = Path(
    os.environ.get("SCRIBE_TOKEN_FILE")
    or Path(os.environ.get("SCRIBE_DATA_DIR") or ROOT / "data") / "mcp-token"
)
NOT_RUNNING = "Open Scribe's Hoard (python -m scribe) so the assistant can reach your recordings."


def _healthy() -> bool:
    try:
        response = httpx.get(f"{BASE_URL}/api/health", timeout=4, trust_env=False)
        return response.status_code == 200 and response.json().get("service") == "scribe-hoard"
    except Exception:
        return False
...
def ensure_running(timeout_s: float = 45.0) -> bool:
    ...
    if _healthy():
        return True
    if os.environ.get("SCRIBE_BRIDGE_AUTOSTART", "1") == "0":
        return False
    port = urlparse(BASE_URL).port or 5185
    env = {**os.environ, "SCRIBE_PORT": str(port), "PORT_STRICT": "1", "PYTHONUNBUFFERED": "1"}
    ...
    logs = Path(os.environ.get("SCRIBE_DATA_DIR") or ROOT / "data") / "logs"
    logs.mkdir(parents=True, exist_ok=True)
    with open(logs / "scribe-app.log", "ab") as out:
        subprocess.Popen([sys.executable, "-m", "scribe"], cwd=ROOT, env=env, stdin=subprocess.DEVNULL,
...
class ScribeBridge(FastMCP):
    ...
    def __init__(self, catalog: list[dict], instructions: str):
        super().__init__(name="scribe-hoard", instructions=instructions)
...
def main() -> None:
    ...
    ScribeBridge(catalog, instructions).run(transport="stdio")
